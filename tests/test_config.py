...
def test_parse_config_reads_safe_display_options(tmp_path: Path) -> None:
    config = parse_config(
        {
            "gallery_label": "Portfolio Gallery",
            "show_diagnostics": True,
        },
        base_dir=tmp_path,
    )

    assert config.gallery_label == "Portfolio Gallery"
    assert config.show_diagnostics is True
...
def test_parse_config_rejects_boolean_thumbnail_size(tmp_path: Path) -> None:
    with pytest.raises(ConfigError):
        parse_config({"thumbnail_size": [True, 320]}, base_dir=tmp_path)


def test_parse_config_rejects_boolean_positive_int(tmp_path: Path) -> None:
    with pytest.raises(ConfigError):
        parse_config({"max_images_per_view": True}, base_dir=tmp_path)


def test_parse_config_rejects_data_dir_inside_projects_root(tmp_path: Path) -> None:
    with pytest.raises(ConfigError, match="data_dir must be outside projects_root"):
        parse_config(
            {
